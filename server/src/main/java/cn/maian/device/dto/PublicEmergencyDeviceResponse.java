...
public record PublicEmergencyDeviceResponse(
    UUID id,
    DeviceType type,
    String category,
    String name,
    ...
    public static PublicEmergencyDeviceResponse from(EmergencyDevice device, Instant now, long freshSeconds) {
        return new PublicEmergencyDeviceResponse(
            ...
            DeviceSchedulePresenter.display(device.getServiceWindows()),
            DeviceSchedulePresenter.windows(device.getServiceWindows()),
            device.getExpireDate(), device.getOwner(),
            device.getVehicleInfo(), device.getServiceRange(), device.getInstructions(),
            device.getImageMediaIds(), device.getVehicleImageMediaIds(), device.getLastLocationAt(),
