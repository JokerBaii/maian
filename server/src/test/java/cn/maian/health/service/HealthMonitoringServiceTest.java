...
class HealthMonitoringServiceTest {
    ...
    void returnsHonestEmptyStateInsteadOfFabricatedMeasurements() {
        ...
        when(readingRepository.findAllByUserIdAndRecordedAtGreaterThanEqualOrderByRecordedAtAsc(
            eq(UserProfileService.CURRENT_USER_ID), any(Instant.class)
        )).thenReturn(List.of());
        when(wearableRepository.findByUserId(UserProfileService.CURRENT_USER_ID))
    ...
    void derivesAlertsFromStoredReadingsAndUserThresholds() {
        ...
        var readings = List.of(
            HeartRateReading.create(UserProfileService.CURRENT_USER_ID, null, 72, "resting", now.minusSeconds(60)),
            HeartRateReading.create(UserProfileService.CURRENT_USER_ID, null, 132, "exercise", now)
        );
        when(readingRepository.findAllByUserIdAndRecordedAtGreaterThanEqualOrderByRecordedAtAsc(
            eq(UserProfileService.CURRENT_USER_ID), any(Instant.class)
        )).thenReturn(readings);
    ...
    private void stubSettings() {
        when(settingsService.findOrCreate())
            .thenReturn(UserSettings.defaults(UserProfileService.CURRENT_USER_ID));
    }
}
